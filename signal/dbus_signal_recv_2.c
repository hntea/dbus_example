...
{  
    ...
    dbus_bus_add_match(bus, "type='signal',interface='com.burtonini.dbus.Signal'",&error);  // 定义匹配器  
...
{  
    ...
    if (dbus_message_is_signal(message, DBUS_INTERFACE_LOCAL,"Disconnected")) // 接收连接成功消息，判断是否连接失败  
    {  
        g_main_loop_quit (loop);              // 退出主循环  
        return DBUS_HANDLER_RESULT_HANDLED;  
    }  
    if (dbus_message_is_signal(message, "com.burtonini.dbus.Signal", "Ping"))   // 指定消息对象路径，判断是否成功  
    {  
